...
def Kelvin_Helmholtz_2D(Lx, Ly, Omega_ce, alpha_e, alpha_i):
    ...
    U0 = 0.01 # Fluid velocity amplitude.
    
    # Wavenumbers.
    ky = 16 * jnp.pi / Ly
    
    # Electron and ion fluid velocities.
    Ue = lambda x, y, z: U0 * jnp.array([jnp.tanh(ky * (y - Ly / 4)) * (jnp.sign(Ly / 2 - y) + 1) / 2
                                         + jnp.tanh(ky * (3 * Ly / 4 - y)) * (jnp.sign(y - Ly / 2) + 1) / 2, 
                                         jnp.zeros_like(x), jnp.zeros_like(x)])
    Ui = lambda x, y, z: U0 * jnp.array([jnp.tanh(ky * (y - Ly / 4)) * (jnp.sign(Ly / 2 - y) + 1) / 2
                                         + jnp.tanh(ky * (3 * Ly / 4 - y)) * (jnp.sign(y - Ly / 2) + 1) / 2, 
                                         jnp.zeros_like(x), jnp.zeros_like(x)])
