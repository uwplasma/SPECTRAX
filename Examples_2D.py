...
def Kelvin_Helmholtz_2D(Lx, Ly, Omega_ce, alpha_e, alpha_i):
    ...
    # Electron and ion fluid velocities.
    Ue = lambda x, y, z: U0 * jnp.array([dU0 * jnp.sin(kx * y / 2), jnp.tanh(kx * (x - Lx / 4)) * (jnp.sign(Lx / 2 - x) + 1) / 2
                                         + jnp.tanh(kx * (3 * Lx / 4 - x)) * (jnp.sign(x - Lx / 2) + 1) / 2, jnp.zeros_like(x)])
    Ui = lambda x, y, z: U0 * jnp.array([dU0 * jnp.sin(kx * y / 2), jnp.tanh(kx * (x - Lx / 4)) * (jnp.sign(Lx / 2 - x) + 1) / 2
                                         + jnp.tanh(kx * (3 * Lx / 4 - x)) * (jnp.sign(x - Lx / 2) + 1) / 2, jnp.zeros_like(x)])
