...
def _make_tail(alpha_s, u_s, Nx, Nn, Nm, Np, Ns, spec, species, mass,
    ...
    def evaluate(Ck):
        result = jax.lax.map(integrate, cells(Ck))
        if not diagnostic:
...
def make_local_tail_objective(alpha_s, u_s, *, Nx, Nn, Nm, Np, Ns, spec,
    ...
    Cellwise map/checkpoint bounds velocity workspace to O(q**3), including AD;
...
def spatial_negative_mass_diagnostics(Ck, alpha_s, u_s, *, Nx, Nn, Nm, Np,
    ...
    negative, fraction, density = jax.lax.map(integrate, cells(Ck))
