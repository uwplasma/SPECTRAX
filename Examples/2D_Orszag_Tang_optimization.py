...
the consistent out-of-plane current, as in ``2D_Orszag_Tang.py``. Any real scalar of the
``simulation`` output can be optimised. Two are provided:
...
Modes::
  ...
  python 2D_Orszag_Tang_optimization.py optimize  --objective conversion --modes 4 --grid 32 --hermite 4
  python 2D_Orszag_Tang_optimization.py benchmark --grid 16 --hermite 4 --t-max 100
  python 2D_Orszag_Tang_optimization.py plot results/optimize_conversion.json results/benchmark.json

Every run writes a JSON report to ``--output``; ``plot`` draws the figures from JSON only.
The reverse pass uses Diffrax's binomial checkpointing (``--checkpoints``), so its memory is
a fixed multiple of the forward solve, independent of the number of time steps.
"""
...
def setup(theta, grid, hermite, t_max, nu=1.0):
    """Initial condition from controls ``theta = (log amplitudes, phases)`` at fixed in-plane magnetic energy."""
    M = theta.size // 2
    k = wavevectors(M)
    a = jnp.exp(theta[:M])
...
OBJECTIVES = {
    ...
    "peak_current": lambda output: -jnp.mean(current_density(output) ** 8) ** (1 / 8),
}


def make_loss(args, M=None):
    M = args.modes if M is None else M
    objective = OBJECTIVES[args.objective]
    return lambda theta: objective(solve(setup(theta, args.grid, args.hermite, args.t_max), args.grid, args.hermite,
                                        args.checkpoints))


def initial_controls(M, seed):
    return jnp.concatenate([jnp.zeros(M), jnp.asarray(np.random.default_rng(seed).uniform(-np.pi, np.pi, M))])
...
def optimize(args):
    ...
    theta0 = initial_controls(args.modes, args.seed)
    start = time.perf_counter()
    ...
    def fun(theta):
        ...
        history.append(dict(iteration=len(history), objective=float(value), seconds=time.perf_counter() - start))
        return float(value), np.asarray(gradient, dtype=float)

    result = minimize(fun, np.asarray(theta0), jac=True, method="L-BFGS-B", options=dict(maxiter=args.iterations))
    report = dict(mode="optimize", settings=vars(args) | {"output": str(args.output)}, initial=np.asarray(theta0).tolist(),
                  optimized=np.asarray(result.x).tolist(), iterations=int(result.nit), evaluations=int(result.nfev),
                  message=str(result.message), history=history, seconds=time.perf_counter() - start, runs={})
    for name, theta in (("initial", theta0), ("optimized", jnp.asarray(result.x))):
        output = solve(setup(theta, args.grid, args.hermite, args.t_max), args.grid, args.hermite, timesteps=41)
    ...
    path = args.output / f"optimize_{args.objective}.json"
...
def benchmark(args):
    ...
    report = dict(mode="benchmark", settings=vars(args) | {"output": str(args.output)}, device=str(jax.devices()[0]),
                  cost=[], memory_steps=[], memory_resolution=[], fd_step=[])
    for M in args.control_counts:
        loss = make_loss(args, M)
        ...
        report["cost"].append(dict(controls=2 * M, forward_s=t_forward, reverse_s=t_reverse, fd_s=t_fd,
                                   fd_relative_error=float(np.linalg.norm(fd - gradient) / np.linalg.norm(gradient))))
        print(f"P={2*M:4d}: forward {t_forward:.2f} s, AD gradient {t_reverse:.2f} s ({t_reverse/t_forward:.1f}x), "
    ...
    theta = initial_controls(args.modes, args.seed)
    ...
    for steps in args.step_counts:
        ...
        parameters = setup(theta, args.grid, args.hermite, steps * dt)
        loss = lambda th, K: OBJECTIVES[args.objective](solve(setup(th, args.grid, args.hermite, steps * dt), args.grid, args.hermite, K, **fixed))
    ...
    for grid, hermite in args.resolutions:
        loss = lambda th: OBJECTIVES[args.objective](solve(setup(th, grid, hermite, args.t_max), grid, hermite, args.checkpoints))
    ...
    for path in paths:
        ...
        if report["mode"] == "optimize":
            ...
            for run, color in (("initial", COLORS["initial"]), ("optimized", COLORS["optimized"])):
                ...
                axes[1, 0].plot(t, np.array(r["inplane_magnetic"]) / r["inplane_magnetic"][0], color=color, label=f"{run} controls")
            ...
            axes[1, 0].set(title="(d) In-plane magnetic energy / initial", xlabel="$t\\,\\omega_{pe}$")
            axes[1, 1].set(title="(e) Kinetic energy change", xlabel="$t\\,\\omega_{pe}$")
            h = report["history"]
            axes[1, 2].plot([r["iteration"] for r in h], [r["objective"] for r in h], color=COLORS["third"], marker="o", ms=3)
            axes[1, 2].set(title=f"(f) Objective '{s['objective']}' vs. L-BFGS iteration", xlabel="gradient evaluation")
            axes[1, 0].legend(frameon=False); axes[1, 1].legend(frameon=False, fontsize=7)
            fig.suptitle(f"Orszag–Tang inverse design: {s['modes']*2} controls, {s['grid']}² × {s['hermite']}³, "
                         f"{runs['optimized']['steps']} Dopri8 steps, {report['seconds']:.0f} s", fontsize=10)
        else:
            ...
            fig.suptitle(f"Gradient cost and memory, {report['settings']['grid']}² × {report['settings']['hermite']}³, {report['device']}", fontsize=10)
        for ext in ("png", "pdf"):
...
def main():
    ...
    parser.add_argument("mode", choices=["optimize", "benchmark", "plot"])
    parser.add_argument("files", nargs="*", help="JSON reports to plot")
    parser.add_argument("--objective", choices=OBJECTIVES, default="conversion")
    parser.add_argument("--modes", type=int, default=4, help="number of stream-function modes M (2M controls)")
