...
\section{Initial conditions used in Fig.\ 1}

\subsection{Fig.\ 1: one-species IC (positivity-safe)}
The script supports three one-species IC families via \code{--fig1\_ic}:
\begin{itemize}[nosep]
  \item \code{poly4} (default): a strongly non-Maxwellian, globally nonnegative ``Maxwellian $\times$ positive polynomial'' IC that excites modes up to $n=4$ (for $n_{\max}\ge 4$).
  \item \code{twostream}: a two-stream-like mixture designed to be nonnegative in physical space; it is projected to the truncated basis and optionally ``high-mode damped'' to remain nonnegative on diagnostic grids.
...
\paragraph{\code{poly4} (default): Maxwellian times a positive polynomial (exact at $n_{\max}\ge 4$).}
The physical-space target is
\begin{align}
  f_{\mathrm{poly4}}(x,y,z)
  = Z\,\psi_0(x)\psi_0(y)\psi_0(z)\Big(1+a_{2x}x^2+a_{2y}y^2+a_{2z}z^2+a_{4x}x^4+a_{4y}y^4+a_{4z}z^4\Big),
\end{align}
with $Z$ chosen so that $f[0,0,0]=1/v_{\mathrm{th}}^3$.
The coefficients are set \emph{exactly as in the code} (see \code{build\_ic\_fig1\_1sp\_poly4}):
\begin{align}
  \texttt{base2}=0.55,\qquad \texttt{base4}=0.32\ \ (\text{if }n_{\max}\ge 4),\qquad a=\code{--amp1},
\end{align}
and then
\begin{align}
  a_{2x}=\texttt{base2}(1+0.60\,a),\quad a_{2y}=a_{2z}=\texttt{base2}(1-0.30\,a),
\end{align}
with the same pattern for the quartic coefficients $a_{4x},a_{4y},a_{4z}$.
Because all polynomial coefficients are nonnegative, the target shape is globally nonnegative in physical space,
and for $n_{\max}\ge 4$ this IC is exactly representable in the truncated Hermite basis.

\paragraph{\code{twostream}: shifted-Maxwellian mixture projected to the truncated basis.}
...
\subsection{Fig.\ 1: two-species IC (positivity-safe, strongly non-Maxwellian)}
The two-species IC is designed to produce a clean temperature-exchange relaxation while keeping both species
noticeably non-Maxwellian \emph{and} nonnegative.
Both species are constructed as
\begin{align}
  f(x,y,z)=Z\,\psi_0(x)\psi_0(y)\psi_0(z)\Big(1+a_2 r^2 + a_4 r^4\Big),\qquad r^2=x^2+y^2+z^2,
\end{align}
with $Z$ chosen so that $f[0,0,0]=1/v_{\mathrm{th}}^3$.

For $n_{\max}\ge 4$ the script uses fixed quartic strengths (see \code{build\_ic\_fig1\_2sp}):
\begin{align}
  a_{4,\mathrm{hot}}=0.14,\qquad a_{4,\mathrm{cool}}=0.08,
\end{align}
and then tunes the quadratic coefficient:
\begin{itemize}[nosep]
  \item the \emph{hot} species uses $a_2\ge 0$ (bisection) so that its temperature matches $T_{\mathrm{eq}}+|\code{--dT2}|$;
  \item the \emph{cool} species uses $a_2\le 0$ (bisection) to keep its temperature near $T_{\mathrm{eq}}$ while
  maintaining global positivity. For $a_4>0$, global positivity for all $r$ is ensured by enforcing $a_2^2\le 4a_4$.
\end{itemize}
