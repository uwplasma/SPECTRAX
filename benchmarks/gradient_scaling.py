...
def worker(config):
    ...
    objective, _ = example.problem(
        ...
        checkpoints=8 if config["method"] == "budgeted" else None)
...
def main():
    ...
    for axis, values in [("grid", options.grids), ("steps", options.steps),
        ...
        for value in values:
            for method in (["checkpointed", "forward", "fd"] if axis == "controls"
                           else ["checkpointed", "budgeted", "taped"]):
                case = dict(axis=axis, method=method, grid=16, steps=64,
                            controls=8, hermite=options.hermite)
                case[axis] = value
                cases.append(case)
    ...
    labels = dict(checkpointed="SOLVAX", budgeted="Fixed budget (K=8)",
                  taped="Taped AD", forward="Forward AD", fd="Finite differences")
    for ax, axis in zip(axes, ("grid", "steps", "controls")):
        positions = []
        for method, color in colors.items():
            ...
            y = [r["seconds"] if axis == "controls" else r["xla_buffer_bytes"] / 2 ** 20 for r in rows]
            positions.extend(x)
            ax.loglog(x, y, "o-", label=labels[method], color=color, ms=4)
        ax.set_xlabel(dict(grid="Plasma state (MiB)", steps="RK4 steps", controls="Control parameters")[axis])
        ax.set_ylabel("Gradient time (s)" if axis == "controls" else "XLA buffer estimate (MiB)")
    ...
    for extension in ("png", "pdf"):
        fig.savefig(options.output / f"gradient_scaling.{extension}", dpi=300)
    # A different gradient is not an acceptable speedup. Compare every matched case.
    failures = []
    for axis in ("grid", "steps", "controls"):
